...
module.exports = {
    async listarUsuarios(request, response) {
        try {
            //instruções SQL
            const sql = `SELECT
            usu_id, usu_nome, usu_email, usu_dt_nasc, usu_senha, usu_tipo, usu_ativo = 1 AS usu_ativo FROM usuarios WHERE usu_ativo = 1;`;
            //executa instruções SQL e armazena o reultado na variavel usuarios
            const usuarios = await db.query(sql);
            //armazena em uma variavel o numero de ristros retornados
    ...
    async cadastrarUsuarios(request, response) {
        try {
            // parâmetros recebudos no corpo da requisição
            ...
            VALUES (?,?,?,?,?,?)`;
            //definição dos dados a sere, inseridos em um array
    ...
    async apagarUsuarios(request, response) {
        try {
            ...
            return response.status(200).json({
                sucesso: true,
                mensagem: `Usuário ${usu_id} exclusão com sucesso`,
